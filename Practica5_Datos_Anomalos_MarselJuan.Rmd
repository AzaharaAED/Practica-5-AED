...
En sepal.length hay un valor atípico (0). En sepal.width aparecen valores negativos y cero, que no tienen sentido. En petal.length también hay valores de 0, y en petal.width se observa un valor infinito, lo cual es inconsistente. Además hay algún que otro NA.
...
dirty_iris <- dirty_iris %>% 
  mutate(across(where(is.numeric),                ~ifelse(!is.finite(.)|.<=0,NA,.)))
...
print(sum(complete.cases(dirty_iris)))
print(sum(complete.cases(dirty_iris))/nrow(dirty_iris))
...
reglasigma <- function(x) {
  ...
  lower <- mu - 3 * sdv
  upper <- mu + 3 * sdv
  condicion <- (x < lower) | (x > upper)              # vector lógico
  return(ifelse(is.na(x), NA, condicion))
}

# Identificador Hampel
reglahampel <- function(x) {
  med <- median(x, na.rm = TRUE)
  madm <- 1.4826 * median(abs(x - med), na.rm = TRUE) # MAD robusta
  lower <- med - 3 * madm
  upper <- med + 3 * madm
  condicion <- (x < lower) | (x > upper)
  return(ifelse(is.na(x), NA, condicion))
}
...
reglapercentil <- function(x) {
  qs <- quantile(x, probs = c(0.05, 0.95), na.rm = TRUE, names = FALSE)
  lower <- qs[1]
  upper <- qs[2]
  condicion <- (x < lower) | (x > upper)
  return(ifelse(is.na(x), NA, condicion))
